...
async function init() {
  ...
  try {
    ...
    const corEl   = document.getElementById('coronal') as HTMLDivElement;   // 👈 NUEVO
    ...
    renderingEngine.setViewports([
      ...
      {
        viewportId: 'CT_CORONAL',             // 👈 NUEVO
        element: corEl,                       // 👈 NUEVO
        type: ViewportType.ORTHOGRAPHIC,
        defaultOptions: { orientation: OrientationAxis.CORONAL }, // 👈 NUEVO
      },
    ]);
    ...
    setVolumesForViewports(
      renderingEngine,
      [{ volumeId }],
      ['CT_AXIAL', 'CT_SAGITTAL', 'CT_CORONAL'] // 👈 NUEVO
    );
    ...
    toolGroup.addViewport('CT_CORONAL', renderingEngineId); // 👈 NUEVO
    ...
    // 12) Render
    renderingEngine.renderViewports(['CT_AXIAL', 'CT_SAGITTAL', 'CT_CORONAL']); // 👈 NUEVO
